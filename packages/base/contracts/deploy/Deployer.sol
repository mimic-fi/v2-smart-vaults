...
import '../permissions/PermissionsData.sol';
...
contract Deployer is Ownable {
    ...
    function deploy(DeployParams memory params) external onlyOwner {
        PermissionsManager manager = new PermissionsManager(address(this));
        emit PermissionsManagerDeployed(address(manager));

        ISmartVault smartVault = _deploySmartVault(params.registry, params.smartVaultParams);
        emit SmartVaultDeployed(address(smartVault));

        _setUpPermissions(manager, smartVault, params.owners);
    }

    /**
     * @dev Internal function to deploy a new smart vault
     * @param registry Address of the Mimic Registry to validate the implementation addresses
     * @param params Deployment params to set up a smart vault
     */
    function _deploySmartVault(IRegistry registry, SmartVaultParams memory params)
        internal
    ...
    /**
     * @dev Internal function to set up permissions for the Smart Vault and Permissions Manager
     ...
     */
    function _setUpPermissions(PermissionsManager manager, ISmartVault smartVault, address[] memory owners) internal {
