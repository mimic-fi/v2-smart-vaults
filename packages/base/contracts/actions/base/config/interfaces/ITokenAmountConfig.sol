...
/**
 * @dev Token amount config interface
 */
interface ITokenAmountConfig is IAuthorizer {
    /**
    ...
    /**
     * @dev Allows defining a list of amount values that should be processed for a given token
     */
    function getTokenAmountHint(address token) external view returns (uint256[] memory);
