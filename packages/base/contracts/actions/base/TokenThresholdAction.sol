...
abstract contract TokenThresholdAction is ITokenThresholdAction, BaseAction {
    ...
    constructor(TokenThresholdConfig memory config) {
        if (config.defaultThreshold.token != address(0)) _setDefaultTokenThreshold(config.defaultThreshold);
        _setTokenThresholds(config.tokens, config.thresholds);
    }
     ...
     */
    function _isThresholdValid(Threshold memory threshold, address token, uint256 amount) internal view returns (bool) {
    ...
    function _beforeAction(address token, uint256 amount) internal virtual override {
        ...
        require(_isThresholdValid(threshold, token, amount), 'ACTION_TOKEN_THRESHOLD_NOT_MET');
    }
     ...
     * will be ignored.
     */
    function _setTokenThresholds(address[] memory tokens, Threshold[] memory thresholds) internal {
     ...
     * @param tokens List of token addresses to unset its custom thresholds
     */
    function _unsetTokenThresholds(address[] memory tokens) internal {
        for (uint256 i = 0; i < tokens.length; i++) {
            address token = tokens[i];
