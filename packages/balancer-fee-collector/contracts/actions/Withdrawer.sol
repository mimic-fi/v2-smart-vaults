...
contract Withdrawer is BaseAction {
    ...
    TokensAcceptance.Config private _recipientsAcceptance;
    ...
    function call(address token, uint256 amount, address recipient) external redeemGas(token) {
        _validate();
        _tokensAcceptance.validate(token);
        _recipientsAcceptance.validate(recipient);
