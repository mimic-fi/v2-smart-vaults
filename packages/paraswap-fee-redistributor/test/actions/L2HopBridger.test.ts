...
describe('L2HopBridger', () => {
  ...
  beforeEach('deploy action', async () => {
    ...
    action = await deploy('L2HopBridger', [
      {
        admin: owner.address,
      ...
      const itPerformsTheExpectedCall = (relayed: boolean) => {
        ...
        context('when the given token has an AMM set', () => {
          ...
          context('when the destination chain ID was set', () => {
            ...
            context('when the slippage is below the limit', () => {
              ...
              context('when the bonder fee is below the limit', () => {
                ...
                const bonderFee = balance.mul(bonderFeePct).div(fp(1))
                ...
                context('when the current balance passes the threshold', () => {
                  ...
                  it('calls the bridge primitive', async () => {
                    const tx = await action.call(token.address, slippage, bonderFee)
                  ...
                  if (relayed) {
                    ...
                    it('does not refund gas', async () => {
                      ...
                      await action.call(token.address, slippage, bonderFee)
                ...
                context('when the current balance does not pass the threshold', () => {
                  ...
                  it('reverts', async () => {
                    await expect(action.call(token.address, slippage, bonderFee)).to.be.revertedWith(
            ...
            context('when the slippage is above the limit', () => {
              ...
              it('reverts', async () => {
                await expect(action.call(token.address, slippage, 0)).to.be.revertedWith('BRIDGER_SLIPPAGE_ABOVE_MAX')
              })
            })
          })
          ...
          context('when the destination chain ID was not set', () => {
            it('reverts', async () => {
              await expect(action.call(token.address, 0, 0)).to.be.revertedWith('BRIDGER_CHAIN_NOT_SET')
            })
          })
        })
        ...
        context('when the given token does not have an AMM set', () => {
          it('reverts', async () => {
            await expect(action.call(token.address, 0, 0)).to.be.revertedWith('BRIDGER_TOKEN_AMM_NOT_SET')
          })
        })
      }
