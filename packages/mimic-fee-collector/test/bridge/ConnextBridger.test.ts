...
describe('ConnextBridger', () => {
  ...
  describe('call', () => {
    ...
    context('when the sender is authorized', () => {
      ...
      context('when the given token is the native token', () => {
        ...
        context('when the destination chain ID was allowed', () => {
          ...
          context('when the given token is allowed', () => {
              ...
              context('when the relayer fee is below the limit', () => {
                ...
                context('when the current balance passes the threshold', () => {
                  ...
                  it('calls the wrap primitive', async () => {
                    const tx = await action.call(chainId, token, amount, relayerFee)
                ...
                context('when the current balance does not pass the threshold', () => {
                  ...
                  it('reverts', async () => {
                    await expect(action.call(chainId, token, amount, relayerFee)).to.be.revertedWith(
            ...
            context('when the requested amount is zero', () => {
              ...
              it('reverts', async () => {
                await expect(action.call(chainId, token, amount, 0)).to.be.revertedWith('BRIDGER_AMOUNT_ZERO')
              })
            })
          })
          ...
          context('when the given token is not allowed', () => {
            it('reverts', async () => {
              await expect(action.call(chainId, token, 0, 0)).to.be.revertedWith('BRIDGER_TOKEN_NOT_ALLOWED')
            })
          })
        })
        ...
        context('when the destination chain ID was not allowed', () => {
          ...
          it('reverts', async () => {
            await expect(action.call(chainId, token, 0, 0)).to.be.revertedWith('BRIDGER_CHAIN_NOT_ALLOWED')
          })
        })
      })
      ...
      context('when the given token is an ERC20 token', () => {
        ...
        context('when the destination chain ID was allowed', () => {
          ...
          context('when the given token was allowed', () => {
              ...
              context('when the relayer fee is below the limit', () => {
                ...
                context('when the current balance passes the threshold', () => {
                  ...
                  it('does not call the wrap primitive', async () => {
                    const tx = await action.call(chainId, token.address, amount, relayerFee)
                ...
                context('when the current balance does not pass the threshold', () => {
                  ...
                  it('reverts', async () => {
                    await expect(action.call(chainId, token.address, amount, relayerFee)).to.be.revertedWith(
                      'MIN_THRESHOLD_NOT_MET'
                    )
                  })
                })
            ...
            context('when the requested amount is zero', () => {
              ...
              it('reverts', async () => {
                await expect(action.call(chainId, token.address, amount, 0)).to.be.revertedWith('BRIDGER_AMOUNT_ZERO')
              })
            })
          })
          ...
          context('when the given token is not allowed', () => {
            it('reverts', async () => {
              await expect(action.call(chainId, token.address, 0, 0)).to.be.revertedWith('BRIDGER_TOKEN_NOT_ALLOWED')
            })
          })
        })
        ...
        context('when the destination chain ID was not allowed', () => {
          ...
          it('reverts', async () => {
            await expect(action.call(chainId, token.address, 0, 0)).to.be.revertedWith('BRIDGER_CHAIN_NOT_ALLOWED')
          })
        })
      })
    })
    ...
    context('when the sender is not authorized', () => {
      it('reverts', async () => {
        await expect(action.call(0, ZERO_ADDRESS, 0, 0)).to.be.revertedWith('AUTH_SENDER_NOT_ALLOWED')
      })
    })
  })
})
